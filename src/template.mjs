...
const rasterIcons={pen:'design',refresh:'update',headset:'support',talk:'conversation',screen:'devices',chart:'report',mail:'contact',doc:'brief'};
export const icon=(name,cls='')=>rasterIcons[name]?'<img class="icon service-art '+cls+'" src="assets/icon-'+rasterIcons[name]+'.webp" width="640" height="640" alt="" aria-hidden="true" loading="lazy" decoding="async">':'<svg class="icon '+cls+'" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="'+paths[name]+'"/></svg>';
const cta=(location,label='LINEで無料相談',cls='')=>'<a class="button '+cls+'" data-contact data-location="'+location+'" href="'+site.contact+'"><span>'+label+'</span>'+icon('arrow')+'</a>';
...
const screen=(s)=>s.screenshot?'<span class="sample-screen screenshot-screen"><span class="sample-status"><span>9:41</span><span>▮▮▮ ▰</span></span><img src="assets/'+s.screenshot+'" width="470" height="910" alt="'+esc(s.name)+'のホームページデザイン" decoding="async"></span>':'<span class="sample-screen theme-'+s.theme+'"><span class="sample-status"><span>9:41</span><span>▮▮▮ ▰</span></span><span class="sample-nav"><b>'+s.name+'</b><span class="sample-menu">☰</span></span><span class="sample-main">'+image(s)+'<span class="sample-copy"><strong>'+lines(s.headline)+'</strong><small>'+esc(s.sub)+'</small><span class="sample-cta">'+s.action+'</span></span></span><span class="sample-bottom">SCROLL TO DISCOVER <span>↓</span></span></span>';
const phone=(s,i)=>'<li class="sample-item"><button class="sample-trigger" data-sample="'+s.id+'" data-title="'+esc(s.name)+'" aria-label="'+s.label+'のサンプル「'+s.name+'」を拡大"><span class="phone-float" style="--order:'+i+'"><span class="phone"><span class="phone-back" aria-hidden="true"><span class="camera-cluster"><i></i><i></i><i></i></span><span>OneBe</span></span><span class="phone-front">'+screen(s)+'<span class="island" aria-hidden="true"></span></span></span></span><span class="sample-caption">'+s.label+'<span>'+icon('arrow')+'</span></span></button></li>';
const schema={
...
<link rel="preload" as="image" href="assets/hero-background.webp"><link rel="canonical" href="${site.url}"><link rel="icon" href="assets/favicon.svg" type="image/svg+xml">
...
<div class="showcase" role="group" id="samples" aria-label="制作サンプル"><div class="carousel-viewport" role="group" tabindex="0" aria-label="円形に配置した制作サンプル。横スクロール、ドラッグ、スワイプ、左右の矢印キーで回せます。"><div class="orbit-scroll-space"><ul class="sample-track">${samples.map(phone).join('')}</ul></div></div>
<div class="carousel-bottom container"><p class="sample-notice">制作イメージ <span>／ 横にスワイプして、ぐるりと見てみる</span></p><div class="carousel-controls"><span class="orbit-counter" aria-hidden="true">01 / 09</span><button class="icon-button" data-prev aria-label="前のサンプル">${icon('arrow','reverse')}</button><button class="text-control" data-spin>${icon('rotate')}<span>1周して見る</span></button><button class="icon-button" data-pause aria-pressed="false" aria-label="スマホの動きを停止">${icon('pause')}</button><button class="icon-button" data-next aria-label="次のサンプル">${icon('arrow')}</button></div></div></div></section>
...
<section class="closing" id="contact" aria-labelledby="closing-title"><div class="container"><p class="section-eyebrow">LET’S TALK</p><h2 id="closing-title">その想いを、<br class="mobile-break">次のお客様へ。</h2><p>ビジネスの魅力をカタチにするホームページを、OneBeと一緒に。</p>${cta('closing','LINEで無料相談','button-light')}</div></section>
</main>
<footer class="site-footer"><div class="container"><div class="footer-top"><a class="brand" href="#top">${logo()}</a><nav aria-label="フッターメニュー"><a href="#features">特徴</a><a href="#samples">制作イメージ</a><a href="#pricing">料金</a><a href="${site.company}">運営者について</a><a href="${site.privacy}">プライバシーポリシー</a></nav></div><div class="footer-bottom"><p>掲載しているサイト・写真は制作イメージです。<br class="mobile-break">実際の制作実績・顧客を示すものではありません。</p><small>© 2026 OneBe</small></div></div>
<a class="mobile-cta" href="${site.contact}" data-contact data-location="mobile" aria-hidden="true" tabindex="-1">LINEで無料相談${icon('arrow')}</a></footer>
<dialog class="sample-dialog" aria-labelledby="dialog-title"><div class="dialog-inner"><button class="dialog-close icon-button" aria-label="サンプルを閉じる">${icon('close')}</button><div class="dialog-copy"><p class="section-eyebrow">DESIGN SAMPLE</p><h2 id="dialog-title">制作サンプル</h2><p class="dialog-category"></p><p>事業の世界観に合わせた、<br>ホームページの制作イメージです。</p><p class="muted">掲載内容は架空のサンプルです。</p><a class="button" href="${site.contact}" data-contact data-location="sample">このデザインをLINEで相談${icon('arrow')}</a></div><div class="dialog-preview"></div></div></dialog>
</body></html>`;}
